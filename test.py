...
def load_training_data(folder_list):
    X_train = []
    ...
    for i in range(len(folder_list)):
        ...
        images = [cv2.imread(f, 0) for f in image_files]
...
def load_data_from_file():
    # X, Y = load_training_data(folder_list, size)
    path = sys.argv[1]
    X_test, files_name = load_training_data([path])

    return X_test , files_name



def predict_from_pickle(model, X_test, file_names):
    ...
    # first collumn is the file name and second collumn is the predicted label
    output = np.column_stack((file_names, output))

    print(output)

    # save the output to a csv file 
    np.savetxt("1705017_predictions.csv", output, delimiter=",", fmt="%s")
...
def main():
    ...
    X_test, files_names = load_data_from_file()
    output = predict_from_pickle(model, X_test, files_names)
